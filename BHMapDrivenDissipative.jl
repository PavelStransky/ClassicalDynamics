...
# bh_dissipation_driving.md. Same Distributed/pmap structure, per-trajectory seeds and resumable
# output files as BHMapDissipative.jl; only the swept parameters and the observables differ.
#
...
@everywhere const INTEGRATION_TIME = 4000.0
...
@everywhere const DELTA_VALUES = LinRange(0.5, 3.0, 501)
@everywhere const F_VALUES = LinRange(0.5, 2.5, 401)
...
@everywhere const NOISE_STEP = 0.01
...
@everywhere function RandomInitialCondition(parameters, rng)
...
""" All trajectories of one (Δ, f) point. `indices` are the global trajectory indices still missing
...
for Δ in DELTA_VALUES
    for f in F_VALUES
        mkpath(PATH)
        file = PATH * "/" * @sprintf("%.3f_%.3f", Δ, f) * ".txt"
        if isfile(file)
