...
# to parallelise across (U, E) pairs instead: one array task computes a
...
#
# The full (U, E) grid is flattened into a single 0-based index, U-major /
...
# cld(length(U_VALUES) * length(ENERGY_VALUES), PAIRS_PER_TASK) tasks --
...
const J = 1.0            # Float64 so modelParameters is a concrete NTuple -> type-stable EquationOfMotion!
const L = 4
...
const U_VALUES = collect(LinRange(0.0, 1.0, 101))
const ENERGY_VALUES = collect(LinRange(-2.25, 2.25, 451))

# One SLURM array task computes a contiguous block of at most PAIRS_PER_TASK
# pairs from the flattened (U, E) grid. One pair takes up to ~5 min, so 100
# pairs stay well under Chimera's 12 h per-task limit.
const PAIRS_PER_TASK = 5

const N_ENERGY = length(ENERGY_VALUES)
const TOTAL_PAIRS = length(U_VALUES) * N_ENERGY
const N_TASKS = cld(TOTAL_PAIRS, PAIRS_PER_TASK)


# Flat 0-based index -> (U, E), U-major / E-minor.
pairAt(k) = (U_VALUES[k ÷ N_ENERGY + 1], ENERGY_VALUES[k % N_ENERGY + 1])

println("Grid: $(length(U_VALUES)) U x $N_ENERGY E = $TOTAL_PAIRS pairs; ",
